/**
 * @fileoverview Applicazione "Gioco della Sfortuna" - Tema Videogiochi
 * @author Jacopo Ricciardi
 * @description Versione digitale con tema Videogiochi del gioco da tavolo "Stuff Happens"
 */
 ...
 * @property {string} id - Identificatore della carta 
 * @property {string} nome - Nome dell'evento sfortunato
 * @property {number} indice - Indice di sfortuna da 1 a 99
 * @property {string} urlImmagine - URL per ottenere  l'immagine
 */
/**
 * @description Dataset completo delle 50 carte del gioco.
 * Ogni carta rappresenta una situazione sfortunata a tema videogiochi,
 * con indice di sfortuna crescente da 1 (banale) a 99 (catastrofico).
 * Gli indici sono tutti univoci, con differenza minima di 2 tra ogni carta.
 * @type {Carta[]}
 */
 ...
 * @param {Object} props - Props del componente
 * @param {Function} props.onPlay - Callback chiamata quando il giocatore avvia la partita
 * @returns {JSX.Element} Schermata home renderizzata
 */
...
const stileHome = StyleSheet.create({
  ...
  title: { fontSize: 38, fontWeight: '900', textAlign: 'center', color: '#fff', textTransform: 'uppercase', letterSpacing: 2, textShadowColor: '#7c3aed', textShadowOffset: { width: 0, height: 0 }, textShadowRadius: 18 },
  subtitle: { fontSize: 16, fontWeight: '700', color: '#b794f6', letterSpacing: 3, marginTop: -8, marginBottom: 20 },
  description: { fontSize: 12, fontWeight: '800', color: 'white', letterSpacing: 1, marginBottom: 20 },
...
/**
 * @function App
 * @description Componente principale che gestisce la navigazione tra le schermate
 * @returns {JSX.Element} Schermata attiva dell'applicazione
 */
export default function App() {
  /** @type {[string, Function]} Schermata attualmente visualizzata */
  const [schermata, setSchermata] = useState('home');
  /** @type {[string|null, Function]} Esito dell'ultima partita */
